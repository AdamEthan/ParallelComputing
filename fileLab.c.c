       ...
       DIR *dir;
         struct dirent *ent;
             ...
             /* Open directory stream */
                 char* path = strtok(dirname, "/");
                 dir = opendir (path);
                 char* lab= "lab2";
                  //list_file((snprintf(buffer, "%s/%s", dirname, ent->d_name,)), ent->d_name, list_long);
                    if (dir != NULL) {
                     printf("%s: \n", dirname);
                     while ((ent = readdir (dir)) != NULL){
                             char* listAChar=ent->d_name;
                             if(ent->d_type==DT_REG && list_all==false && listAChar[0]!='.')
                                     printf ("%s\n", ent->d_name);
...
int main(int argc, char* argv[]) {
    ...
    // TODO: Replace this.
      if(optind==argc){
     list_dir("./",list_long,list_all,false);
    }
    if(argc>1){
     for(int i=optind;i<argc;i++){
    list_dir(argv[i],list_long,list_all,false);
    }
    }
...
static void help() {
    ...
    printf("ls: List files\n");
    printf("\t--help: Print this help\n");
    printf("Type ./ls to list all directories");
    printf("Type tag -l for more information");
    printf("Type tag -r to list all files recursively including subdirectories");
    printf("Type tag -a to list all files including hidden files within a directory");
    exit(0);
}
...
bool is_dir(char* pathandname) {
    /* TODO: fillin */
    if(test_file(pathandname)==true){
     DIR* dir=opendir(pathandname);
     if(dir){
     closedir(dir);
     return true;
     }
